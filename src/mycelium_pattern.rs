...
pub fn generate_mycelium_pattern(rng: &mut RandomStream, width: usize, height: usize, iteration: usize) -> Vec<Vec<char>> {
    ...
    for _ in 0..initial_points {
        ...
        if start_x < width && start_y < height {
            ...
            if rng.next_f64() < (0.05 + iteration as f64 * 0.005).min(0.5) { // Increased chance for emoji
    ...
    let inner_iterations = (iteration as f64 * 0.5).min(20.0) as usize; // Max 20 inner iterations
                ...
                if grid[y][x] != ' ' { // If a cell has mycelium or a node
                    ...
                    for (dx, dy) in &neighbors {
                        ...
                        if nx < width && ny < height && new_grid[ny][nx] == ' ' {
                            ...
                            if rng.next_f64() < (0.05 + iteration as f64 * 0.008).min(0.6) { 
                                if rng.next_f64() < (0.02 + iteration as f64 * 0.003).min(0.2) { // Increased chance to place an emoji
                                    let emoji_str = EMOJIS[rng.gen_range(0..=EMOJIS.len() - 1)];
                                ...
                                if rng.next_f64() < (0.005 + iteration as f64 * 0.001).min(0.1) { // Increased probability for new nodes
